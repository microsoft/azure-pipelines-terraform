...
export abstract class BaseTerraformCommandHandler {
    ...
    public async show(): Promise<number> {
        ...
        if(outputTo == "console"){
            ...
            if (outputFormat == "json") {
                ...
                // execSync resolves on a non-zero exit code where execAsync would reject.
    ...
    public async custom(): Promise<number> {
        ...
        if(outputTo == "console"){
            ...
            const customFilePath = path.resolve(tasks.getInput("workingDirectory") || tasks.getVariable('System.DefaultWorkingDirectory') || '.', tasks.getInput("filename", true));
            let commandOutput = await terraformTool.execSync(<IExecSyncOptions> {
                cwd: customCommand.workingDirectory});
