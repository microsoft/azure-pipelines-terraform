...
export default class TerraformPlanDisplay extends React.Component<{ attachments: IAttachmentService }> {
    ...
    // Formatting walks and clones the whole plan, which is far too expensive to repeat
    // on every render (the observer re-runs for unrelated state such as the dropdown).
    private formattedPlans = new Map<number, string>();
    ...
    private getFormattedPlan(index: number, planText: string): string {
        let html = this.formattedPlans.get(index);

        if (html === undefined) {
            html = formatPlanForDisplay(planText);
            this.formattedPlans.set(index, html);
        }
                    ...
                    <Observer chosenPlan={this.chosenPlan} plans={this.plans} plansLoaded={this.plansLoaded} hasError={this.hasError}>
                        {(props: { chosenPlan: number, plans: TerraformPlan[], plansLoaded: boolean, hasError: boolean }) => {
                            ...
                            if (!props.plansLoaded) {
                                ...
                                html = this.getFormattedPlan(props.chosenPlan, props.plans[props.chosenPlan].plan);
                            }
